...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    tikzplotlib.save(os.path.join(result_plot_path, '{}_plot.tex'.format(plot_type)))
    fig.savefig(os.path.join(result_plot_path, '{}_plot.pdf'.format(plot_type)), dpi=300, format='pdf', transparent=True)
    plt.draw()
...
def check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation=False, **kwargs):
    ...
    result_plot_path = os.path.join(settings.result_plot_dir, report_name)
...
def generate_formatted_report(row_labels, scores, table_name=''):
    # Short display names for score columns
    short_names = {'Precision': 'PRE', 'Norm Precision': 'NPR', 'AUC': 'AUC'}
    name_width = max([len(d) for d in row_labels] + [len(table_name)]) + 5

    # Header
    header_cols = [short_names.get(k, k) for k in scores.keys()]
    report_text = '\n{label: <{width}} | {cols} |'.format(
        label=table_name, width=name_width, cols=' | '.join(header_cols))
    report_text += '\n'

    # Data rows
    for trk_id, d_name in enumerate(row_labels):
        score_strs = ['{:0.2f}'.format(v[trk_id].item()) for v in scores.values()]
        report_text += '{tracker: <{width}} | {scores} |\n'.format(
            tracker=d_name, width=name_width, scores=', '.join(score_strs))
...
def plot_got_success(trackers, report_name):
    ...
    for trk_id, trk in enumerate(trackers):
        json_path = os.path.join(settings.got_reports_path, '{}.json'.format(trk.name))
