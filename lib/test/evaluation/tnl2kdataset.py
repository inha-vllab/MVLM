...
class TNL2kDataset(BaseDataset):
    ...
    def __init__(self):
        super().__init__()
        self.base_path = os.path.join(self.env_settings.tnl2k_dir, 'test')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        text_dsp_path = os.path.join(self.base_path, sequence_name, 'language_rebuttal_simple.txt')
        try:
            text_dsp = load_str(text_dsp_path)
        except:
            text_dsp_path = os.path.join(self.base_path, sequence_name, 'language.txt')
            text_dsp = load_str(text_dsp_path)


        frames_path = os.path.join(self.base_path, sequence_name, 'imgs')
        frames_list = [f for f in os.listdir(frames_path)]
        frames_list = sorted(frames_list)
        frames_list = [os.path.join(frames_path, frame_i) for frame_i in frames_list]
