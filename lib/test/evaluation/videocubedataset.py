import os
import numpy as np
from lib.test.evaluation.data import BaseDataset, SequenceList
...
class VideoCubeDataset(BaseDataset):
    ...
    def __init__(self, split, version='full'):
        ...
        self.base_path = self.env_settings.mgit_dir

    def get_sequence_list(self):
        return SequenceList([self._construct_sequence(s) for s in self.sequence_list])

    def _construct_sequence(self, sequence_name):
        anno_path = os.path.join(self.base_path, 'attribute', 'groundtruth', sequence_name + '.txt')

        ground_truth_rect = load_text(str(anno_path), delimiter=',', dtype=np.float64)

        nlp_path = os.path.join(self.base_path, 'mgit_nlp', sequence_name + '.xlsx')
        nlp_tab = pd.read_excel(nlp_path)
        nlp_rect = nlp_tab.iloc[:, [14]].values
        nlp_rect = nlp_rect[-1, 0]

        frames_path = os.path.join(self.base_path, 'data', self.split, sequence_name)
        frame_list = [frame for frame in os.listdir(frames_path) if frame.endswith(".jpg")]
        frame_list.sort(key=lambda f: int(f[:-4]))
        frames_list = [os.path.join(frames_path, frame) for frame in frame_list]

        return VideoCube_Sequence(sequence_name, frames_list, 'videocube', ground_truth_rect.reshape(-1, 4), object_class=None, target_visible=None, language_query=nlp_rect)

    def __len__(self):
        return len(self.sequence_list)

    def _get_sequence_list(self, split):
        path = os.path.join(self.base_path, 'data', split + '_list.txt')
