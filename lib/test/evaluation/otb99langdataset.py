...
class OTB99LangDataset(BaseDataset):
    ...
    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.otb99_lang_dir
    ...
    def _construct_sequence(self, sequence_info):
        ...
        frames = [os.path.join(self.base_path, 'OTB_videos', sequence_path,
                              '{frame:0{nz}}.{ext}'.format(frame=frame_num, nz=nz, ext=ext))
                  for frame_num in range(start_frame + init_omit, end_frame + 1)]

        anno_path = os.path.join(self.base_path, 'OTB_videos', sequence_info['anno_path'])

        # NOTE: OTB has some weird annos which panda cannot handle
        ground_truth_rect = load_text(str(anno_path), delimiter=(',', None), dtype=np.float64, backend='numpy')

        nlp_path = os.path.join(self.base_path, 'OTB_query_test', '{}.txt'.format(sequence_name))
        nlp_rect = load_text(str(nlp_path), delimiter=',', dtype=str)
        nlp_rect = str(nlp_rect)

        return Sequence(sequence_info['name'], frames, 'otb99_lang', ground_truth_rect[init_omit:, :],
