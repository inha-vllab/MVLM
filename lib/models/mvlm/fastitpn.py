...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.use_language_token_mask = use_language_token_mask
        self.template_free = template_free
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, text_emb):
        ...
        if text_emb is not None:
            xz = torch.cat([xz, text_emb], dim=1)
    ...
    @staticmethod
    def create_language_token_mask(B, N_x, N_text, device):
        """
        Create mask to block self-attention among text_emb tokens
        B: batch size
        N_x: number of x tokens
        N_text: number of text_emb tokens
        """
        total_len = N_x + N_text

        # Initialize full attention matrix (allow all positions)
        attn_mask = torch.ones(B, total_len, total_len, device=device)

        # Block self-attention within text_emb region
        # Set text_emb region [N_x:N_x+N_text, N_x:N_x+N_text] to 0 for all batches
        attn_mask[:, N_x:N_x+N_text, N_x:N_x+N_text] = 0

        return attn_mask

    def forward_features(self, template_list, search_list, text_emb):

        xz = self.prepare_tokens_with_masks(template_list, search_list, text_emb)
        
        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None

        num_of_lang = text_emb.shape[1] if text_emb is not None else 0
        num_of_cls = 1 if self.cls_token is not None else 0
        
        tokens_before_encoder = xz[:, num_of_cls:, :].clone()
        
        num_template = len(template_list)
        
        num_of_x = self.num_patches_search
        num_of_z = self.num_patches_template * num_template if not self.template_free else 0
        
        if self.use_language_token_mask:
            attn_mask = self.create_language_token_mask(xz.shape[0], num_of_cls + num_of_x + num_of_z, num_of_lang, xz.device)
        else:
            attn_mask = None

        for _, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            xz = blk(xz, rel_pos_bias, attn_mask)

        x = xz[:, num_of_cls:num_of_cls + num_of_x, :]
        if not self.template_free:
            z = xz[:, num_of_cls + num_of_x:num_of_cls + num_of_x + num_of_z, :]
            
        tokens_after_encoder = xz[:, num_of_cls:, :].clone()
        
        lang_token = xz[:, -num_of_lang:, :] if self.use_language_token_mask else None
        ...
        out_dict = {
            ...
            'tbe': tokens_before_encoder,
            'tae': tokens_after_encoder,
        }
    ...
    def forward(self, template_list, search_list, text_emb):
        return self.forward_features(template_list, search_list, text_emb)
