...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                if load_previous_ckpt:
                    directory = os.path.join(self._checkpoint_dir, self.settings.project_path_prv)
                    self.load_state_dict(directory)
                if distill:
                    directory_teacher = os.path.join(self._checkpoint_dir, self.settings.project_path_teacher)
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if epoch > (max_epochs - 3) or save_every_epoch or epoch % 10 == 0:
    ...
    def save_checkpoint(self):
        ...
        # Exclude frozen CLIP weights (text_encoder.clip.*) to reduce checkpoint size.
        # The trainable projection (text_encoder.text_proj.*) is kept.
        # CLIP is always re-loaded from pretrained weights via clip.load() at init.
        net_state = {k: v for k, v in net.state_dict().items()
                     if not k.startswith('text_encoder.clip.')}
        state = {
            'epoch': self.epoch,
        ...
        tmp_file_path = os.path.join(directory, '{}_ep{:04d}.tmp'.format(net_type, self.epoch))
        torch.save(state, tmp_file_path)

        file_path = os.path.join(directory, '{}_ep{:04d}.pth.tar'.format(net_type, self.epoch))

        # Now rename to actual checkpoint. os.replace is cross-platform atomic.
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        if checkpoint is None:
            ...
            checkpoint_list = sorted(glob.glob(os.path.join(self._checkpoint_dir,
                                                                             self.settings.project_path, '{}_ep*.pth.tar'.format(net_type))))
            if checkpoint_list:
            ...
            checkpoint_path = os.path.join(self._checkpoint_dir, self.settings.project_path,
                                                                 '{}_ep{:04d}.pth.tar'.format(net_type, checkpoint))
        elif isinstance(checkpoint, str):
            # checkpoint is the path
            if os.path.isdir(checkpoint):
                checkpoint_list = sorted(glob.glob(os.path.join(checkpoint, '*_ep*.pth.tar')))
                if checkpoint_list:
                    checkpoint_path = checkpoint_list[-1]
                else:
